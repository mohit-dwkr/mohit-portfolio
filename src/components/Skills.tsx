      ...
      <div className="container-narrow">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
          {skills.map((skill, index) => (
            <div
              key={skill.name}
              className="group card-elevated p-6 md:p-8 text-center opacity-0 animate-fade-in"
              style={{ animationDelay: `${0.1 * index}s` }}
            >
              <div className="flex justify-center mb-4 text-foreground group-hover:text-muted-foreground transition-colors duration-300">
                {skill.icon}
              </div>
              <h3 className="heading-card mb-2">{skill.name}</h3>
